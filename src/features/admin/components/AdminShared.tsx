...
export const SectionAccordion = ({
  ...
  return (
    <details open={defaultOpen} className="motion-enter group overflow-hidden rounded-[1.3rem] border border-mist/60 bg-[linear-gradient(180deg,rgba(22,28,38,0.98),rgba(11,15,20,0.98))] shadow-lift">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3.5 sm:px-5 sm:py-4">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-xs text-slate-400 sm:text-sm">{subtitle}</p>
        </div>

        <div className="flex items-center gap-2">
          {badge}
          <div className="flex h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] text-slate-300 transition group-open:rotate-180 sm:h-10 sm:w-10">
            <ChevronDown size={18} />
          </div>
        </div>
      </summary>

      <div className="border-t border-white/8 px-4 py-3.5 sm:px-5 sm:py-4">{children}</div>
    </details>
  );
};
