...
import { ExerciseLog } from '../../../shared/types';
import { useUI } from '../../../app/providers/ui-context';
import { fetchExerciseLogsForDate, upsertExerciseLog } from '../../../shared/api/dataApi';
...
export const useExerciseLogs = (
    ...
    await Promise.all(
      payloads.map(async ([logId, payload]) => {
        try {
          ...
          // ignore write errors for optimistic logs
        }
      })
    );
  ...
  const updateExerciseLog = async (log: ExerciseLog) => {
    try {
      ...
      setLogs((prevLogs) => {
        const existingIndex = prevLogs.findIndex((entry) => (
          entry.exerciseId === logWithAllFields.exerciseId
          && entry.userId === logWithAllFields.userId
          && entry.date === logWithAllFields.date
        ));
